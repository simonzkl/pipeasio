...
static int die_asio(IPipeASIO *asio, const char *what, LONG err)
{
    char msg[124] = {0};
    ...
    if (asio) {
        ...
        if (msg[0])
            fprintf(stderr, "[probe] driver error: %s\n", msg);
    }
    return die(what, err);
}
...
int main(void)
{
    ...
    if (rc == 0) {
        ...
        if (rc != 0) {
            int ret = die_asio(asio, "SetSampleRate(48000)", rc);
    ...
    if (rc != 0) {
        int ret = die_asio(asio, "CreateBuffers", rc);
    ...
    if (rc != 0) {
        int ret = die_asio(asio, "Start", rc);
